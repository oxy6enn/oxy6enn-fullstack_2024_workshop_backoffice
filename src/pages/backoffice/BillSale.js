...
function BillSale() {
    ...
    const [billSaleDetails, setBillSaleDetails] = useState([]) //form BillSaleDetail Table
    ...
    const handleSend = async (item) => {
        try {
            const button = await Swal.fire({
                ...
                text: 'ยืนยันการจัดส่งสินค้าใช่หรือไม่?',
                icon: 'question',
                showCancelButton: true,
                showConfirmButton: true
            })
    ...
    const handleCancel = async (item) => {
        try {
            const button = await Swal.fire({
                ...
                text: 'ยืนยันการยกเลิกใช่หรือไม่?',
                icon: 'question',
                showCancelButton: true,
                showConfirmButton: true
            })
    ...
    const displayStatusText = (item) => {
        if (item.status === 'wait') {
            ...
            return <div className='badge bg-info'>ชำระแล้ว</div>
        } else if (item.status === 'send') {
            return <div className='badge bg-success'>จัดส่งแล้ว</div>
